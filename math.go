...
func f(x int, coefs_bytes []int) int {
	if x == 0 {
...
func multiply_polynomials(a, b []int) []int {
...
func full_lagrange(xs, fxs []int) []int {
	...
	for i := 0; i < len(fxs); i++ {
		...
		for j := 0; j < len(fxs); j++ {
			...
			this_term := []int{gf256_div(xs[j], denominator), gf256_div(1, denominator)}
		...
		this_polynomial = multiply_polynomials(this_polynomial, []int{fxs[i]})
